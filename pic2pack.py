...
class pic2packMod(loader.Module):
	...
	async def разрезатьcmd(self, message):
		""".разрезать {название} + <ответ на фото>"""
		
		reply = await message.get_reply_message()
		if not reply:
			await message.edit("<b>Ответ на фото❗</b>")
			return
		
		args = utils.get_args_raw(message)
		if not args:
			await message.edit("<b>Название пака</b>❓")
			return
		chat = '@Stickers'
		...
		w, h = image.size 
		www = max(w, h)
		await message.edit("🔪<b>Режу тебя...</b>")
		...
		face.save(fface)
		fface.seek(0)
		await message.edit("<b>Уже разобрал на куски, теперь собираю в пак...</b>")
			...
			try:
				...
				for part in ending.raw_text.split():
					if part.startswith("https://t.me/"):
						break
				await message.edit('✅<b>Все, я собрал тебя по частям вот ты - </b>\n'+part)
