...
class FiltersMod(loader.Module):
    ...
    async def filtercmd(self, message):
        ...
        await message.edit((
            "<b>Триггер добавлен.</b>".format(name)))
        message.message = ""
    ...
    async def stopcmd(self, message):
        ...
        chatid = str(message.chat_id)
        if not filtern:
            await message.edit(("<b>Укажите название триггега</b>"))
            return
        try:
            del filters[chatid][filtern]
            await message.edit(("<b>Триггер </b><i>{}</i><b> успешно удален из чата.</b>".format(filtern)))
            self._db.set("FilterModule", "filters", filters)
        except KeyError:
    ...
    async def stopallcmd(self, message):
        ...
        try:
            del filters[chatid]
            self._db.set("FilterModule", "filters", filters)
            await message.edit(("<b>Все триггеры успешно удалены.</b>"))
        except KeyError:
    ...
    async def filterscmd(self, message):
        ...
        try:
            ...
            pass
        filterl = "<b>Триггеры чата: </b>\n\n{}".format(filters)
        if filters:
            await message.edit(filterl)
        else:
            await message.edit(("<b>В этом чате нет триггеров.</b>"))
